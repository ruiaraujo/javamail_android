...
/**
 * This class contains a single method that does authentication using SASL. This
 * is in a separate class so that it can be compiled with J2SE 1.5. Eventually
...
public class IMAPSaslAuthenticator implements SaslAuthenticator {
	...
	public IMAPSaslAuthenticator(IMAPProtocol pr, String name,
	...
	public boolean authenticate(String[] mechs, final String realm,
			final String authzid, final String u, final String p)
		...
		synchronized (pr) { // authenticate method should be synchronized
			...
			/*
			 * Wrap a BASE64Encoder around a ByteArrayOutputstream to craft b64
			 * encoded username and password strings
			 * 
			 * Note that the encoded bytes should be sent "as-is" to the server,
			 * *not* as literals or quoted-strings.
			 * 
			 * Also note that unlike the B64 definition in MIME, CRLFs should
			 * *not* be inserted during the encoding process. So, I use
			 * Integer.MAX_VALUE (0x7fffffff (> 1G)) as the bytesPerLine, which
			...
			boolean isXGWTRUSTEDAPP = sc.getMechanismName().equals(
					"XGWTRUSTEDAPP")
					&& PropUtil.getBooleanProperty(props, "mail." + name
							+ ".sasl.xgwtrustedapphack.enable", true);
				...
				try {
					...
					if (r.isContinuation()) {
						...
						if (ba == null) {
							...
							if (isXGWTRUSTEDAPP)
								bos.write(ASCIIUtility
										.getBytes("XGWTRUSTEDAPP "));
							bos.write(ba);
			...
			/*
			 * Dispatch untagged responses. NOTE: in our current upper level
			 * IMAP classes, we add the responseHandler to the Protocol object
			 * only *after* the connection has been authenticated. So, for now,
			 * the below code really ends up being just a no-op.
			 */
			...
			/*
			 * If we're using the Novell Groupwise XGWTRUSTEDAPP mechanism we
			 * always have to issue a LOGIN command to select the user we want
			 * to operate as.
			 */
			if (isXGWTRUSTEDAPP) {
				...
				// If the response includes a CAPABILITY response code, process
				// it
